...
func main() {
	...
	// Define the route for the scraping API
	router.GET("/api/name", getName)
	router.GET("/api/assignments", getAssignments)
	router.GET("/api/info", getInfo)
	router.GET("/api/averages", getAverages)
	router.GET("/api/classes", getClasses)
	router.GET("/api/reportcard", getReport)
	router.GET("/api/ipr", getProgressReport)
	...
	router.GET("/help", func(c *gin.Context) {
		c.JSON(200, gin.H{"message":"This is the home page, visit the documentation at https://homeaccesscenterapi-docs.vercel.app/", "routes": []string{"/api/name", "/api/assignments", "/api/info", "/api/averages", "/api/classes", "/api/reportcard", "/api/ipr"}})
	})
